...
l1t::PFJet makeJet(std::vector< edm::Ptr< l1t::PFCandidate >> parts){

    l1t::PFCandidate seed = *parts.at(0);

    auto sumpt = [](float a, const edm::Ptr<l1t::PFCandidate>& b){
        return a + b->pt();
    };
...
void L1SeedConePFJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    std::sort(work.begin(), work.end(), [](edm::Ptr<l1t::PFCandidate> i,edm::Ptr<l1t::PFCandidate> j){return(i->pt() > j->pt());});   
    ...
    while(!work.empty() && jets.size() < _nJets){
        ...
        std::copy_if(work.begin(), work.end(), std::back_inserter(particlesInCone), [&](const edm::Ptr<l1t::PFCandidate> &part){
        ...
        work.erase(std::remove_if(work.begin(), work.end(), [&](const edm::Ptr<l1t::PFCandidate> &part){

...
private:
  ...
  std::vector<int>*   m_trk_nSkippedLayers;
  std::vector<bool>*  m_trk_fromPV;
...
void L1TrackNtupleMaker::beginJob()
{
  ...
  m_trk_nSkippedLayers = new std::vector<int>;
  m_trk_fromPV = new std::vector<bool>;
  ...
  if (SaveAllTracks) {
    ...
    eventTree->Branch("trk_nSkippedLayers", &m_trk_nSkippedLayers);
    eventTree->Branch("trk_fromPV", &m_trk_fromPV);
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (SaveAllTracks) {
	...
	tmp_trk_d0 = -tmp_trk_x0*sin(tmp_trk_phi) + tmp_trk_y0*cos(tmp_trk_phi);
      ...
      int tmp_trk_nSkippedLayers = (int) iterL1Track->getNSkippedLayers();
      bool tmp_trk_fromPV = (bool) iterL1Track->getFromPV();
	...
	if (tmp_trk_combinatoric) cout << " (is combinatoric)" << endl; 
      ...
      m_trk_nSkippedLayers->push_back(tmp_trk_nSkippedLayers);
      m_trk_fromPV->push_back(tmp_trk_fromPV);
